...
func TestResolveExecutionContextAsTextOutput(t *testing.T) {
	...
	assert.Equal(t, 300+200+10, output.Usage.InputTokens, "expected input tokens to match")
	assert.Equal(t, 2200+200+100, output.Usage.OutputTokens, "expected output tokens to match")
	assert.Equal(t, 20+20+20, output.Usage.CachedTokens, "expected cached tokens to match")
	assert.Equal(t, 20+0+0, output.Usage.ReasoningTokens, "expected reasoning tokens to match")
